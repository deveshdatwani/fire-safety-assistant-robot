...
class Score():
	...
	def __init__(self):
		...
		# Costmap image without obstacles
		self.FILE_PATH = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/00 ground_truth_costmap.pgm" #update the file location accordingly
		self.FILE_PATH_RRT = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/00 ground_truth_costmap.pgm"
		# Costmap image with obstacles
		self.FILE_PATH2 = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/02 mymap_case2.pgm" #update the file location accordingly
		# Change detect map
		self.FILE_PATH_CHANGE = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/change_map/change_map_case2.png" #update the file location accordingly
		# Ground truth Change detect map
		# self.FILE_PATH_CHANGE_gnd = "/home/aka/capstone_ws/src/autonomous_tb/saved_maps/gnd_trth_change_map.png" #update the file location accordingly
		...
		# Draw map flag - 
		'''update it to true to generate Informed RRT* maps'''
		self.draw_map = False 
		...
		self.map_scores_no_obs = self.path_score(self.start_coordinates, self.goal, self.FILE_PATH_RRT)
		...
		print("Map scores no obstacles {x-coordinate of start point: (path length, path nodes)}:")		
		print(self.map_scores_no_obs)
		print("Map scores with obstacles {x-coordinate of start point: (path length, path nodes)}:")
		...
		self.obs_gradient_score = self.grad_score(self.grad_coordinates, self.goal, self.FILE_PATH, self.FILE_PATH_CHANGE, self.kernel_size, self.sigma, self.max_cost)
	...
	def room_cost(self, y1,y2,x1,x2,combined_map):
		tes = []
		for i in range(x1,x2):
			for j in range(y1,y2):
				tes.append(combined_map[i][j])
		return tes
	...
	def grad_score(self, locations, goal, grnd_trth_file_path, change_map_file_path, k_size, sig, m_cost):	
		...
		test = np.array(change_map_array)

		#Elementwise multiplication of the change detection map with the gradient of the room
		array_multiplication = change_map_array * grad_map_cropped
		combined_map = np.array(array_multiplication)

		# Defining the coordinates of the room (Future work: generate coordiantes of each room automatically)
		room1 = self.room_cost(108,154,97,182,combined_map)
		room2 = self.room_cost(108,154,185,280,combined_map)
		room3 = self.room_cost(154,256,97,208,combined_map)
		room4 = self.room_cost(256,303,185,208,combined_map)
		room5 = self.room_cost(256,302,97,185,combined_map)
		room6 = self.room_cost(306,390,97,208,combined_map)
		room7 = self.room_cost(351,402,208,308,combined_map)

		# Output of gradient score at each room
		print("Gradient scores of each room with obstacles")
		print("Room 1:", np.sum(room1))
		print("Room 2:", np.sum(room2))
		print("Room 3:", np.sum(room3))
		print("Room 4:", np.sum(room4))
		print("Room 5:", np.sum(room5))
		print("Room 6:", np.sum(room6))
		print("Room 7:", np.sum(room7))

		end_time = time.time()
		print("Total time for calculating scores is %.2f" %(end_time-start_time))
		
		# plt.imshow(array_multiplication)
		# plt.show()
